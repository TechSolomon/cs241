// CS 241 Lab 3 Assignment 2: Joystick-driven digits
...
// NOTE: If you attempt to go above/below the
// array value(s) werid digits appear (overflow).
int patternOrder[] = {1, 2, 3, 4, 5, 8, 7, 9};
...
// Order in list; holds information for pattern.
auto patternLocation = 0;

// If flag is false, it will allow user to change input.
bool flagX = false;
bool flagY = false;

// True is ON – false is OFF for digits.
bool rightDigit;
bool leftDigit;
...
void loop() {
  ...
  showChar(patternOrder[patternLocation]);
  // Changing Digit
  ...
  if (yVal == 2 && flagY == false) {
    if (patternLocation == sizeof(patternOrder))
    {
      patternLocation = 0;
    }
    else {
    ...
    if (patternLocation == 0) {
      patternLocation = sizeof(patternOrder);
    }
    else {
      patternLocation--;
    }
